...
export const updateUser = async (updatedUser: User, userId: User["id"]) => {
  try {
    if (updatedUser.password != "") {
      ...
      const query =
        "UPDATE users SET photo=?, name=?, email=?, phone=?, startDate=?, state=?, password=?, jobDescription=?, position= ? WHERE id=?";
      ...
      const query =
        "UPDATE users SET photo=?, name=?, email=?, phone=?, startDate=?, state=?,jobDescription=?, position= ? WHERE id=?";
      const userDb = (await queryDb(query, [
        updatedUser.photo,
        updatedUser.name,
        updatedUser.email,
